...
public:
    ...
    ~IRContext() {
        spvContextDestroy(syntax_context_);
    }
    ...
    void SetMessageConsumer(MessageConsumer c) {
        consumer_ = std::move(c);
    }
    ...
    bool AreAnalysesValid(Analysis set) {
        return (set & valid_analyses_) == set;
    }
    ...
    void ResetFeatureManager() {
        feature_mgr_.reset(nullptr);
    }

    // Returns the grammar for this context.
    const AssemblyGrammar& grammar() const {
        return grammar_;
    }
    ...
    uint32_t max_id_bound() const {
        return max_id_bound_;
    }
    void set_max_id_bound(uint32_t new_bound) {
        max_id_bound_ = new_bound;
    }

    bool preserve_bindings() const {
        return preserve_bindings_;
    }
    void set_preserve_bindings(bool should_preserve_bindings) {
        preserve_bindings_ = should_preserve_bindings;
    }

    bool preserve_spec_constants() const {
        return preserve_spec_constants_;
    }
    ...
    spv_target_env GetTargetEnv() const {
        return syntax_context_->target_env;
    }
